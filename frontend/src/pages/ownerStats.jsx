...
import OwnerPieChart from "../utility/ownerPieChart";
    ...
    useEffect(() => {
        if (userId) {
            ...
            axios.get(`http://localhost:8080/properties/count/by-owner/${userId}`)
                .then(res => setTotalPropertyCount(res.data))
                .catch(err => console.error(err));
            ...
            axios.get(`http://localhost:8080/properties/occupancyRate/by-owner/${userId}`)
                ...
                .catch(err => console.error(err));
            axios.get(`http://localhost:8080/properties/averageRent/by-owner/${userId}`)
                .then(res => setavgRent(res.data))
                .catch(err => console.error(err));
            axios.get(`http://localhost:8080/payments/yearlyRent/by-owner/${userId}`)
                .then(res => setYearlyRent(res.data))
                .catch(err => console.error(err));
            axios.get(`http://localhost:8080/properties/rentedProperties/by-owner/${userId}`)
                .then(res => setRentedPropertyCount(res.data))
                .catch(err => console.error(err));

            if (TotalpropertyCount && RentedpropertyCount) {
                setAvailablePropertyCount(TotalpropertyCount - RentedpropertyCount);
            }
        }
    }, [userId]);
                ...
                <div className="flex w-[100%] justify-between h-fit gap-[10px] pl-[80px] pr-[50px] mt-[10px]">
                    <div className="flex bg-white px-[20px] py-[20px] gap-[30px] rounded-[15px] w-[33vw]">
                        <div className="w-full">
                            ...
                            <div className="flex justify-between">
                                <div>
                                    <h1 className="text-[#232323] text-[24px] font-bold">{TotalpropertyCount ? TotalpropertyCount.toLocaleString() : "0"}                                    </h1>
                    ...
                    <div className="flex bg-white px-[20px] py-[20px] gap-[30px] rounded-[15px] w-[33vw]">
                        <div className="w-full">
                            ...
                            <div className="flex justify-between">
                                <div>
                                    <h1 className="text-[#232323] text-[24px] font-bold">{RentedpropertyCount ? RentedpropertyCount.toLocaleString() : "0"}                                    </h1>
                                    <p className="text-[#949494] text-[12px]">Properties</p>
                                </div>
                                <img className="w-[45px] h-[45px]" src="icons/Home-Icon.svg" alt="" />
                            </div>
                    ...
                    <div className="flex bg-white px-[20px] py-[20px] gap-[30px] rounded-[15px] w-[33vw]">
                        <div className="w-full">
                            <h1 className="text-[#343434] text-[20px]">Available Properties</h1>
                            <div className="flex justify-between">
                                <div>
                                    <h1 className="text-[#232323] text-[24px] font-bold">{AvailablepropertyCount ? AvailablepropertyCount.toLocaleString() : "0"}                                    </h1>
                                    <p className="text-[#949494] text-[12px]">Properties</p>
                                </div>
                                <img className="w-[45px] h-[45px]" src="icons/Home-Icon.svg" alt="" />
                            </div>
