...
export default function Histogram() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch data for user "ahmed88"

        const response = await axios.get(`http://localhost:8080/api/v1/payments/monthly-payments/${userName}`);
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          ...
          console.warn("No valid registration data received.");
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-md w-full h-[40vh]">
      <div className="flex justify-between items-center mb-[10px]">
        <h2 className="text-3xl font-bold text-gray-900">Monthly Payments</h2>
